...
```bash
uvicorn task4.main:app --reload --host 0.0.0.0 --port 8000
...
- [docker-compose.yml](docker-compose.yml) — service to launch Postgres with `seed.sql`.
- [requirements.txt](requirements.txt) — Python dependencies.
- [seed.sql](seed.sql) — SQL seed data.
- [task3/main.py](task3/main.py) — Streamlit app for interactive Text→SQL.
- [task3/database.py](task3/database.py) — DB helpers used by the Streamlit app.
- [task4/main.py](task4/main.py) — FastAPI server exposing the agent API.
- [task4/agent.py](task4/agent.py) — agent pipeline using Groq + LangGraph.

If you want, I can:
- Run the app locally (start Docker and launch Streamlit / Uvicorn).
- Redact any secrets found in the repo (e.g., the `.env` file currently contains a GROQ API key).
- Add CONTRIBUTING or developer notes with common troubleshooting steps.
