    ...
    <div className={`message-row ${isUser ? 'user-row' : 'ai-row'}`}>
      <div className={`message-bubble ${isUser ? 'user-bubble' : 'ai-bubble'}`}>
        ...
        <div className="message-meta">
          {msg.time}
          {!isUser && msg.status && (
            <span className={`agent-status-badge status-${msg.status}`}>
              {msg.status.replace(/_/g, ' ')}
            </span>
          )}
          {!isUser && msg.iterations !== undefined && msg.iterations > 0 && (
            <span className="iter-tag">
              🔄 {msg.iterations} step{msg.iterations > 1 ? 's' : ''}
            </span>
          )}
          {!isUser && msg.model && (
...
export default function App() {
  const [messages, setMessages] = useState([
    {
      role: 'ai',
      content: 'Hello! I am your production-grade AI assistant. Ask me anything, or toggle Agent Mode for multi-step cross-source verification.',
      time: formatTime(),
      model: '',
      cache_hit: false,
      fallback: false
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const endpoint = agentMode ? `${API_BASE}/agent/chat` : `${API_BASE}/chat`
      const payload = agentMode
        ? {
            message: text,
            model_type: modelType,
            temperature,
            selected_sources: selectedSources.length > 0 ? selectedSources : null
          }
        : {
            message: text,
            model_type: modelType,
  ...
  return (
    <div className="layout">
      ...
      <aside className="sidebar">
        ...
        <div className="toggle-row agent-toggle-row">
          <div>
            <span style={{ fontWeight: 600, display: 'block' }}>Agent Mode</span>
            <span style={{ fontSize: '11px', color: 'var(--muted)' }}>
              {agentMode ? 'Cross-source verification loop' : 'Single-pass RAG baseline'}
            </span>
          </div>
          <div
            className={`toggle ${agentMode ? 'toggle-on' : ''}`}
        ...
        {!agentMode && (
          <div className="toggle-row">
            ...
            <div
              className={`toggle ${useRag ? 'toggle-on' : ''}`}
      ...
      <main className="chat-area">
        <div className="chat-header">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <span className="sub">
                {agentMode
                  ? 'Task 3: Bounded Cross-Source Verification Agent'
                  : 'W15 Baseline: Single-pass Production RAG'}
              </span>
            </div>
            {agentMode ? (
              <span className="mode-pill agent-pill">
                ✦ Agent Loop Active (/agent/chat)
              </span>
            ) : (
              <span className="mode-pill legacy-pill">
                Single-Pass Mode (/chat)
              </span>
            )}
          </div>
        </div>
        ...
        <div className="input-bar">
          <textarea
            ...
            placeholder={
              agentMode
                ? "Ask a question (agent will search and verify sources)... (Enter to send)"
